    ...
    <div class="min-h-screen p-4 mt-16">
        <div class="grid grid-cols-1">
            <div class="w-auto shadow-sm card bg-white-custom dark:bg-dark-black-custom">
                <div class="card-body">
                    <h3 class="mb-4 text-base font-medium lg:text-xl text-black-custom dark:text-white-custom">Edit Ujian
                    </h3>
                    <form action="{{ route('master_ujian.update', $ujian->id) }}" method="POST" id="form-ujian">
                        @csrf
                        @method('PUT')
                        <div class="mb-4">
                            ...
                            <select name="id_mapel" id="jurusan"
                                ...
                                @foreach ($mapel as $item)
                                    <option value="{{ $item->id }}"
                                        {{ old('id_mapel', $data->id_mapel ?? '') == $item->id ? 'selected' : '' }}>
                                        {{ $item->nama_mapel }}
                                    </option>
                                @endforeach
                            </select>
                        ...
                        {{-- <div class="mb-4">
                            <label for="acak-jawaban"
                                class="block mb-3 text-sm font-medium text-black-custom dark:text-white-custom">Acak Jawaban</label>
