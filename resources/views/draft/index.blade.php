            ...
            <div class="w-auto shadow-sm card bg-white-custom dark:bg-dark-black-custom">
                <div class="card-body">
                    ...
                    <div class="flex flex-col-reverse justify-between w-full gap-3 mb-5 md:gap-0 md:flex-row">
                        <form method="GET" action="{{ route('master_draft.index') }}" id="filterForm">
                            <select
                        ...
                        <table class="table table-zebra">
                            ...
                            <tbody class="text-black-custom dark:text-white-custom">
                                ...
                                @foreach ($kelas as $index => $item)
                                    <tr>
                                        <th>{{ $loop->iteration + ($kelas->currentPage() - 1) * $kelas->perPage() }}</th>
                                        <td>{{ $item->nama_ujian ?? '-' }}</td>
                                        <td>{{ $item->nama_kelas ?? '-' }}</td>


                                        <td class="flex items-center justify-center gap-2">
                                            <a href="{{ route('master_draft.show', base64_encode($item->id_kelas)) }}"
                                                class="border border-black btn btn-edit text-black-custom dark:text-white-custom"><i
                                                    class="fa-regular fa-eye"></i></a>
                                            <form action="{{ route('master_draft.destroy', $item->id_kelas) }}"
